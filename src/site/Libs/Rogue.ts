namespace Rogue
{
	export class BitCanvas
	{
		...
		public inCanvas( x: number, y: number ) { return !( x < 0 || this.w <= x || y < 0 || this.h <= y ); }
		...
		{
			if ( x < 0 || this.w <= x ) { return false; }
			if ( y < 0 || this.h <= y ) { return false; }
			return this.canvas[ y * this.w + x ];
		}

		public getBits() { return this.canvas; }
...
console.log(this.map.toString());
			...
			// TODO: Big room. pop route -> join room
...
console.log(this.map.toString());
		...
		public getCanvas() { return this.map.getCanvas(); }
	...
	class Map
	{
		...
		public getCanvas() { return this.map; }
		...
		public disconnect( route: number, rooms: Room[] )
		{
			this.route.disconnect( route );
			if ( this.route.checkRoute( rooms ) ) { return true; }
		...
		private fixSplit( list: number[] = [], split: number, max: number )
		{
			const nlist: number[] = [];
			let count = 0;
	...
	class Room
	{
		...
		public removed() { return this.area._w <= 0 && this.area._h <= 0; }
		...
		public setDrawArea( w: number, h: number, x: number, y: number )
		{
		...
		{
			if ( this.area._w <= 0 || this.area._h <= 0 ) { return; }
	...
	class Route
	{
		...
		private init()
		{
			...
			for ( let y = 0 ; y < this.h ; ++y )
			{
				...
				{
					if ( 0 < x ) { this.connect( this.calcRoomNumber( x - 1, y ), this.calcRoomNumber( x, y ) ); }
					if ( 0 < y ) { this.connect( this.calcRoomNumber( x, y - 1 ), this.calcRoomNumber( x, y ) ); }
				}
			}
		}
